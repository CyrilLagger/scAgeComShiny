  ...
  dt <- reactive(
    {
      ...
      setnames(
        dt,
        old = c("ID", "LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE", "LOG2FC_ALL", "BH_P_VALUE_DE",
...
choose_TCA_lri <- function(
  ...
  renderUI({
    ...
    choices <-
      c(ALL_LRI_LABEL,
        sort(unique(DATASETS_COMBINED[[input$TCA_DATASET_CHOICE]]@cci_detected[["LRI"]]))
      )
    selectizeInput(
...
get_TCA_interaction_table <- function(
  ...
  DT::renderDT({
    ...
    if (!('All LRI' %in% input$TCA_LRI_CHOICE)) {
      dt <- dt[`LRI` %in% input$TCA_LRI_CHOICE]
    }
    setorder(
      dt,
      -LOG2FC_ALL,
...
plot_TCA_VOLCANO <- function(
  ...
  renderPlot({
    ...
    dt <- dt[, c("LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE",  "LOG2FC", "minus_log10_pval", "Age Regulation")]
    show_volcano(
      data = dt,
      xlims = xlims,
      ylims = ylims
...
get_TCA_VOLCANO_text <- function(
  ...
  renderPrint({
    ...
    dt[, minus_log10_pval := -log10(`BH_P_VALUE_DE` + 1E-4)]
    dt <- dt[, c("ID", "LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE", "REGULATION", "LOG2FC_ALL", "minus_log10_pval")]
    setnames(
      dt,
      old = c("ID", "LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE", "REGULATION", "LOG2FC_ALL"),
...
plot_TCA_SCORES <- function(
  ...
  renderPlot({
    ...
    dt <- dt[, c("LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE",  "LOG2FC_ALL", "REGULATION", "CCI_SCORE_YOUNG", "CCI_SCORE_OLD")]
    setnames(
      dt,
...
get_TCA_SCORES_text <- function(
  ...
  renderPrint({
    ...
    dt <- dt[, c("ID", "LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE",  "LOG2FC_ALL", "REGULATION", "CCI_SCORE_YOUNG", "CCI_SCORE_OLD")]
    setnames(
      dt,
      old = c("ID", "LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE", "REGULATION", "CCI_SCORE_YOUNG", "CCI_SCORE_OLD", "LOG2FC_ALL"),
...
plot_TCA_LRIFC <- function(
  ...
  renderPlot({
    ...
    dt <- dt[, c("ID", "LRI", "EMITTER_CELLTYPE", "RECEIVER_CELLTYPE",  "LOG2FC_ALL", "REGULATION", "LOG2FC_L", "LOG2FC_R")]
    setnames(
      dt,
...
get_TCA_global_table <- function(
  ...
  DT::renderDT({
    ...
    if (categ == "Tissues") {
      ...
      categ <- "ER_CELLFAMILIES_GLOBAL"
      old_cols <- c("ER_CELLFAMILIES","ORA_SCORE_DOWN", "ORA_SCORE_UP", "N_LR")#, "N_LR_YOUNG", "N_LR_OLD")
      new_cols <- c("Cell-Type Family", "ORA Score Down", "ORA Score Up", "LR N.")#, "LR N. Young", "LR N. Old" )
    } else if (categ ==  "LR Genes") {
      categ <- "LRI_GLOBAL"
      old_cols <- c("LRI", "ORA_SCORE_DOWN", "ORA_SCORE_UP", "N_ID", "N_ID_DOWN", "N_ID_UP", "N_ORA_ID_DOWN", "N_ORA_ID_UP",
...
choose_TCA_lri_summary <- function(
  input
) {
  renderUI({
    selectizeInput(
      inputId = 'TCA_LRI_CHOICE',
      label = 'Choose or enter a LRI',
      choices = sort(unique(LRI_REGULATION_SUMMARY$LRI)),
      multiple = FALSE
    )
  })
}
